...
async function loadHelpersWithCountingKeychain(
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "claude-auth-cache-"))
...
describe("exported helpers", () => {
  before(async () => {
    const tempDir = await mkdtemp(join(tmpdir(), "claude-auth-"))
    const tempKeychain = join(tempDir, "keychain.ts")
...
`,
  ...
  it("system transform does not inject when system already contains prefix", async () => {
    ...
    const tempHome = await mkdtemp(join(tmpdir(), "claude-auth-home-"))
