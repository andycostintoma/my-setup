...
const plugin: Plugin = async () => {
  ...
  try {
    ...
    console.warn(
      "claude-auth: Failed to read Claude Code credentials:",
      error,
    )
    return {}
  }
  ...
  if (accounts.length === 0) {
    ...
    console.warn(
      "claude-auth: No Claude Code credentials found. " +
  ...
  if (initialCreds) {
    ...
    console.warn(
      "claude-auth: Claude credentials are expired and could not be refreshed. Run `claude` to re-authenticate.",
    )
  }
