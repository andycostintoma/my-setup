...
function readClaudeConfigPrimaryApiKey(): ClaudeCredentials | null {
  try {
    const configPath = join(homedir(), ".claude.json")
    const raw = readFileSync(configPath, "utf-8")
    const parsed = JSON.parse(raw) as { primaryApiKey?: unknown }

    if (typeof parsed.primaryApiKey !== "string") {
      log("claude_config_primary_key_read", {
        success: false,
    ...
    const creds = parseCredentials(parsed.primaryApiKey)
    log("claude_config_primary_key_read", {
      success: creds !== null,
      hasPrimaryApiKey: true,
    })
    return creds
  } catch {
    log("claude_config_primary_key_read", { success: false })
    return null
  }
}
...
export function readAllClaudeAccounts(): ClaudeAccount[] {
  if (process.platform !== "darwin") {
    const creds = readCredentialsFile() ?? readClaudeConfigPrimaryApiKey()
    if (!creds) return []
