...
**`command not found: ov`** → On this machine, OpenViking is provided by Nix/Home Manager. Ask the user to run `make -C ~/.config/my-setup switch`; do not install it with `pip` or `uv tool`.
