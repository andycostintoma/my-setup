...
`, edgeVersion, edgeURL, edgePrefetch.Hash, kumospaceVersion, kumospaceURL, kumospacePrefetch.Hash)

	if err := os.WriteFile(filepath.Join(*repo, "modules", "pins.nix"), []byte(pins), 0o644); err != nil {
		return err
	}
	fmt.Printf("Updated local pins: microsoft-edge %s, kumospace %s\n", edgeVersion, kumospaceVersion)
	return nil
}
